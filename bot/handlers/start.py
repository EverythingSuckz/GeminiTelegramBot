...
async def log_users(_, message: types.Message):
    asyncio.create_task(db.aou_user(message.from_user.id, gfn(message.from_user), message.from_user.username))
...
async def about_cmd(_, message: types.Message):
    markup = types.InlineKeyboardMarkup([[
        types.InlineKeyboardButton(text="Source Code", url="https://github.com/EverythingSuckz/GeminiTelegramBot"),
    ]])
    await message.reply(
        "I'm a <b>conversational bot</b> powerd by <b>Google's Gemini API</b>.",
        parse_mode=enums.ParseMode.HTML,
        reply_markup=markup,
    )
